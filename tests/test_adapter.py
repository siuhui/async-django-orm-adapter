...
from async_django_orm_adapter.models import CasbinRule
from async_django_orm_adapter import Adapter
...
def get_enforcer():
    ...
    sync_to_async(CasbinRule.objects.bulk_create)(
        [
...
class TestConfig(TestCase):
    ...
    async def test_add_policy(self):
        ...
        await adapter.add_policy(sec=None, ptype="p", rule=["alice", "data1", "read"])
        await adapter.add_policy(sec=None, ptype="p", rule=["bob", "data2", "write"])
        await adapter.add_policy(sec=None, ptype="p", rule=["data2_admin", "data2", "read"])
        await adapter.add_policy(sec=None, ptype="p", rule=["data2_admin", "data2", "write"])
        await adapter.add_policy(sec=None, ptype="g", rule=["alice", "data2_admin"])
