...
class Adapter(persist.Adapter):
    ...
    async def load_policy(self, model):
        ...
        try:
            lines = await sync_to_async(CasbinRule.objects.using(self.db_alias).all)()
    ...
    @staticmethod
    def _create_policy_line(ptype, rule):
    ...
    async def save_policy(self, model):
        ...
        await sync_to_async(CasbinRule.objects.using(self.db_alias).all().delete)()
        ...
        rows_created = await sync_to_async(CasbinRule.objects.using(self.db_alias).bulk_create)(lines)
    ...
    async def remove_policy(self, sec, ptype, rule):
        ...
        rows_deleted, _ = await sync_to_async(CasbinRule.objects.using(self.db_alias).filter(**query_params).delete)()
